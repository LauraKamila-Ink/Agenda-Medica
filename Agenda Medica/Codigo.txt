.data
prompt_nombre:      .asciiz "Ingrese el nombre del archivo: "
nombre_archivo:     .space 256
buffer_entrada:     .space 4096
buffer_salida:      .space 4096
byte_temp:          .space 1
tamano_salida:      .word 0
tamano_original:    .word 0
checksum_w:         .word 0
rc_w:               .word 0

msg_ok:             .asciiz "Archivo le�do y validado correctamente.\n"
msg_noexist:        .asciiz "Error: el archivo no existe o no se pudo abrir.\n"
msg_invalido:       .asciiz "Error: formato no soportado (caracteres inv�lidos).\n"
msg_size:           .asciiz "Error: el archivo excede el tama�o m�ximo de 4 KiB.\n"
msg_rle:            .asciiz "\nResultado de la compresi�n RLE:\n"
msg_no_efectivo:    .asciiz "La compresi�n no fue efectiva.\n"
msg_checksum:       .asciiz "\nChecksum: "
msg_rc:             .asciiz "\nRC (%): "
nombre_salida:      .asciiz "salida.bin"
...
main:
    ...
    # Leer y validar archivo (ahora guarda tamano_original)
    jal validar_y_leer
    bltz $v0, salir_error
    ...
    # Calcular checksum
    jal calcular_checksum

    # Calcular RC
    jal calcular_RC

    # Guardar salida en archivo binario
    jal guardar_salida

    # Mostrar checksum y RC
    li $v0, 4
    la $a0, msg_checksum
    syscall
    li $v0, 1
...
# Quita el salto de l�nea '\n' del nombre

limpiar_nombre:
    la $t0, nombre_archivo
limpiar_loop:
    lb $t1, 0($t0)
    beqz $t1, limpiar_fin
    beq $t1, 10, limpiar_null
    addi $t0, $t0, 1
    j limpiar_loop
limpiar_null:
    sb $zero, 0($t0)
...
# Leer archivo y validar caracteres
# Guarda tamano_original
...
leer_loop:
    ...
    li $a2, 1
    syscall
    beqz $v0, fin_read    # EOF (0 bytes read)

    lb $t3, byte_temp
    blt $t3, 0x20, err_invalido
    bgt $t3, 0x7E, err_invalido

    sb $t3, 0($t0)
    addi $t0, $t0, 1
    addi $t1, $t1, 1
    bgt $t1, 4096, err_size
    j leer_loop
...
fin_read:
    ...
    sw $t1, tamano_original

    # opcional: terminador nulo en buffer_entrada
    sb $zero, 0($t0)
...
# Compresi�n RLE con control de efectividad
# Escribe also un terminador nulo al final de buffer_salida

comprimir_RLE:
    la $t0, buffer_entrada
    la $t1, buffer_salida
    lb $t2, 0($t0)
    beqz $t2, fin_RLE       # vac�o
...
loop_RLE:
    lb $t4, 0($t0)
    beq $t4, $t2, inc_contador
    beqz $t4, escribir_par

escribir_par:
    ble $t3, 255, escribir_uno
frag_loop:
    li $t5, 255
    sb $t5, 0($t1)
    addi $t1, $t1, 1
...
escribir_uno:
    # Evitar sobrepasar buffer_salida (4 KiB)
    la $t9, buffer_salida
    sub $t6, $t1, $t9
    bge $t6, 4096, no_efectivo
...
fin_RLE:
    # poner terminador nulo en buffer_salida (pos actual de $t1)
    sb $zero, 0($t1)

    # Calcular tama�os
    la $t6, buffer_salida
    sub $t7, $t1, $t6       # tama�o salida

    la $t8, buffer_entrada
    sub $t9, $t0, $t8       # tama�o original (pero no fiable si usamos pointers)
    # Usamos la palabra guardada tamano_original para el original
    lw $t9, tamano_original

    # Guardar tama�o de salida
    sw $t7, tamano_salida

    # Comparar tama�os
    bge $t7, $t9, no_efectivo
    jr $ra

# Si la compresi�n no fue efectiva
no_efectivo:
    li $v0, 4
    la $a0, msg_no_efectivo
    syscall
    jr $ra



# Mostrar contenido del buffer_salida
# Interpreta parejas [count, char]

mostrar_RLE:
    la $t0, buffer_salida
loop_mostrar:
    lb $t1, 0($t0)
    beqz $t1, fin_mostrar    # terminador nulo

    # imprimir count (entero)
    li $v0, 1
    move $a0, $t1
    syscall
...
# Calcular checksum (suma bytes mod 256) usando tamano_original

calcular_checksum:
    lw $t0, tamano_original
    beqz $t0, chk_zero
...
# Calcular RC (porcentaje entero)
# RC = (original - comprimido) * 100 / original

calcular_RC:
    lw $t0, tamano_original   # original
    beqz $t0, rc_zero
    lw $t1, tamano_salida     # comprimido
    sub $t2, $t0, $t1         # original - comprimido
    li $t3, 100
    mul $t2, $t2, $t3         # (orig - comp)*100
    div $t2, $t0
    mflo $t4                  # porcentaje entero
    sw $t4, rc_w
    jr $ra
rc_zero:
    li $t4, 0
...
# Guardar buffer_salida en archivo binario 'salida.bin'
